...
**Pin**:
A per-shade absolute override inside a generated ramp: the stop key's presence in a color's `pins` map (stop name → bare hex) *is* the pin; clearing is deleting the key, falling back to the generated shade. Pins survive source-color changes and regeneration; a pinned stop carries no state variables of its own — its value reaches an interaction state only when a palette token references the stop, through the palette-tier `color-mix()` (ADR 0026), not stop-level band logic. Colors-only — sizes deliberately have no pins (see the ADR 0012 amendment and ADR 0019).
...
**Ramp**:
The color tier — an open set of **source colors** (a base hue, e.g. `primary`, `neutral`, custom `magenta`) crossed with an open ordered **stop scale** (an L-ranked position, e.g. `ultra-light`…`ultra-dark`), emitting `--{color}` and `--{color}-{stop}`. Source colors are one **flat** open set (ADR 0024's base-spec-plus-custom model; `brand`/`neutral`/`semantic` groups dissolved to optional editor tags). The **spec authors the emitted names** — a Tailwind spec emits `--primary-100…900`, the base spec `--primary-ultra-light…ultra-dark` — while labels are free and L values are the site's (ADR 0023). Emission is **dense** (presence-is-membership: color × stop, no `enabled`, no holes) because `tokens.css` is an open surface whose references can't be enumerated; only the internal-consumer **state tier** (`-hover`/`-active`) can be reference-gated — and it is, off the ramp entirely (ADR 0026). See ADR 0025.
_Avoid_: colorway, shade toggle, `enabled` flag, hue section, `--semantic-success` (grouped variable names)

**State tier**:
The `-hover`/`-active` interaction-state variables. They emit **only at the palette tier** — the contrast steps (ADR 0015) and intents (ADR 0016) — never on raw ramp stops, because their only consumers are palettes, a **closed internal set** the generator reads (the mirror of the ramp's forced density: internal consumers can be reference-gated, external ones can't). The emitted count is `(palette tokens) × 2`, independent of ramp size — the bound spec §5 asked for. Each state is a **generator-authored `color-mix()`** off the resolved role color (`color-mix(in srgb, var(--palette-accent), black 12%)`): the generator chooses pole and percentage per color (a static component overlay can't branch on the base's lightness; a `color-mix()` expression stays cascade-live where a frozen literal or a stepped `var()` wouldn't). `hover` and `active` are two tokens, active a slightly larger mix; the large "pressed" cues (ring, inset) are structural, a component concern. Exact pole/percentage/chroma-floor tuning is the deferred state-derivation effort. See ADR 0026.
_Avoid_: per-stop hover/active, ramp-stepped states, `color_interaction_shifts` band math (for states), state layer (as a token)

**Naming convention**:
The pattern a spec's keys happen to follow — t-shirt (`s/m/l`), ordinal (`1/2/3`), descriptive (`tight/roomy`), Tailwind-step (`100/200/300`) — an **emergent** property of authored keys, never a modeled type: no enum, no picker, no per-family `convention` field (all four already pass ADR 0021's slug grammar and sort by integer `position`). Naming is entirely spec territory — key, **label**, and emitted variable are all spec data; anticustom imposes none of them. Emission is **key-identity**: the variable is `--{key}` verbatim, any namespace embedded in the key the spec author typed (`space-m → --space-m`, `h1 → --h1`), the generator adding no family prefix (sharpening ADR 0023's `spacing.m → --space-m`). The label is a separate spec-owned display string that may differ from the key. A project switches convention by **relabel** (display only — free, non-breaking, available to a follower) or **reslug** (changes the variable — a drafting act that breaks following skins); cross-convention interop is ADR 0023 aliasing. anticustom ships one canonical base spec, not convention variants. A **duplicate key** (two tokens → one variable) is a hard error; *mixing* conventions within a family is unconventional-but-valid and unguarded. See ADR 0027.
_Avoid_: convention picker/enum, convention field, generator-applied prefix, rank-numbered heading variable, bulk reslug, canonical convention (as a system-enforced thing)

**Heading level**:
The semantic HTML element a heading instance emits — a per-instance `level` field on the heading-emitting components (`intro`, `card`, `hero`), a **different axis** from visual rank (ADR 0022's size-keyed derived styles) and from token naming/rank (ADR 0027's `--{key}` identity): an `h2` that looks like heading-4, or a `p` carrying heading styling, are both expressible. Values are the literal emitted tag (`h1`–`h6`, `p`; labels "Heading 1"…"Paragraph"), so render is a plain `<{level}>`; `p` styles the text as a heading but keeps it **out of the document outline** (rank without level — the fix for heading-spam). Each component declares its own default (`intro`/`hero` `h2`, `card` `h3`); the field sets the tag of the *post-promotion* heading element, composing with eyebrow-promotion (which decides *which* text is the heading, while `level` decides *what tag*). anticustom never **auto-derives** the level — the outline algorithm is dead (`<section>` nesting carries no heading semantics) and component nesting ≠ outline depth — it **validates** instead: the `heading-order` failure state (layout tier, whole assembled DOM) flags a missing/duplicate `h1`, a first heading that isn't the `h1`, or a descending skip >1 (≡ axe-core `page-has-heading-one` + `heading-order`), advisory to editors and enforced at the publish gate; `p`-level instances are absent from the sequence. See ADR 0028, and the **Failure state** / **Advisory vs Enforced** terms.
_Avoid_: auto-derived level, section-derived level, heading rank (as the element), outline algorithm (as a live mechanism), hN coupled to visual size
